...
namespace caffe {
  ...
  __global__ void BOXPoolingForward(
    ...
    CUDA_KERNEL_LOOP(index, nthreads) {
      ...
      int rfcn_r = (index / num_features) % num_rfcn_regions;
      int s = (index / num_features / num_rfcn_regions) % num_samples;
      int r = (index / num_features / num_rfcn_regions / num_samples) % num_regions;
      int m = index / num_features / num_rfcn_regions / num_samples / num_regions;
      int x = seed_points[ 2 * m ];
      int y = seed_points[ 2 * m + 1 ];
      int feat_ext_idx = feat_ext_offset [ 2 * r * num_features + 2 * s ] + x;
      int feat_ext_idy = feat_ext_offset [ 2 * r * num_features + 2 * s + 1] + y;
      int bottom_idx = ((rfcn_r * num_features + f) * height + feat_ext_idy) * width + feat_ext_idx;
      top_data[index] += bottom_data[bottom_idx];
      mapping_channel[ index ] = bottom_idx;
    }
  }
  ...
  __global__ void BOXPoolingBackwardAtomic(
    ...
    const int height, const int width,
    Dtype* bottom_diff, const int num_rfcn_regions) {
    CUDA_KERNEL_LOOP(index, nthreads) {
      // The input is in order (1, num_rfcn_regions * num_features , H, W ), nf = 64
      // sampled_points is in order (m, 2)
      // feat_ext_offset is in order (num_regions, num_samples, 2)
      ...
      for (int i = 0; i < M * num_regions * num_samples; i++ ){
        ...
        int top_idx = (((m * num_regions + r)* num_samples + s) * num_rfcn_regions + rfcn_r)*num_features + f ;
        if (mapping_channel[top_idx] == index){
  ...
  void RegionPoolingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    BOXPoolingBackwardAtomic<Dtype> << <CAFFE_GET_BLOCKS(count),
      ...
      num_features_, num_regions_,num_samples_,  height_, width_, bottom_diff,num_rfcn_regions_);
    CUDA_POST_KERNEL_CHECK;
  }
