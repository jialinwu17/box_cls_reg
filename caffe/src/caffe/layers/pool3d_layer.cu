...
__global__ void MaxPoolForward(const int nthreads, const Dtype* bottom_data,
    ...
    const int kernel_height,const int kernel_width, const int kernel_depth, const int stride, const int temporal_stride, Dtype* top_data) {
...
__global__ void AvePoolForward(const int nthreads, const Dtype* bottom_data,
    const int num, const int channels, const int length, const int height,
    const int width, const int pooled_length, const int pooled_height, const int pooled_width,
    const int kernel_height,const int kernel_width ,const int kernel_depth, const int stride, const int temporal_stride, const int pad, Dtype* top_data) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    int pw = index % pooled_width;
    int ph = (index / pooled_width) % pooled_height;
    int pl = (index / pooled_width / pooled_height) % pooled_length;
    ...
    int lstart = pl * temporal_stride;
    int hend = min(hstart + kernel_height, height + pad);
    int wend = min(wstart + kernel_width, width + pad);
    int lend = min(lstart + kernel_depth, length);
...
void Pooling3DLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom, 
  ...
  case PoolingParameter_PoolMethod_MAX:
    ...
    MaxPoolForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        ...
        height_, width_, pooled_length_, pooled_height_, pooled_width_, kernel_height_,kernel_width_, kernel_depth_,
...
__global__ void MaxPoolBackward(const int nthreads, const Dtype* bottom_data,
    ...
    const int num, const int channels, const int length, const int height,
    const int width, const int pooled_length, const int pooled_height, const int pooled_width,
    const int kernel_height,const int kernel_width, const int kernel_depth, const int stride, const int temporal_stride, Dtype* bottom_diff) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    int phstart = (h < kernel_height) ? 0 : (h - kernel_height) / stride + 1;
    int phend = min(h / stride + 1, pooled_height);
    int pwstart = (w < kernel_width) ? 0 : (w - kernel_width) / stride + 1;
...
__global__ void AvePoolBackward(const int nthreads, const Dtype* top_diff,
    const int num, const int channels, const int length, const int height,
    const int width, const int pooled_length, const int pooled_height, const int pooled_width,
    const int kernel_height,const int kernel_width, const int kernel_depth, const int stride, const int temporal_stride, const int pad,
    Dtype* bottom_diff) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    // find out the local index
    // find out the local offset
...
void Pooling3DLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  case PoolingParameter_PoolMethod_MAX:
    ...
    MaxPoolBackward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        ...
        pooled_width_, kernel_height_,kernel_width_, kernel_depth_, stride_, temporal_stride_, bottom_diff);
    break;
  case PoolingParameter_PoolMethod_AVE:
    // NOLINT_NEXT_LINE(whitespace/operators)
